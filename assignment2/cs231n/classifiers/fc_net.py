...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1,len(dims)):
          ...
          if (self.normalization == "batchnorm" or self.normalization == "layernorm") and i != len(dims)-1:
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          ...
          if self.normalization == "batchnorm":
            X, cache = affine_bn_relu_forward(X, w, b, self.params[f"gamma{i}"],
                                  self.params[f"beta{i}"], self.bn_params[i-1])
          elif self.normalization == "layernorm":
            X, cache = affine_ln_relu_forward(X, w, b, self.params[f"gamma{i}"],
                                  self.params[f"beta{i}"], self.ln_params[i-1])
          else:
        ...
        for i in range(self.num_layers-1, 0, -1):
          ...
          if self.normalization == "batchnorm":
            dout, dw, db, grads[f"gamma{i}"], grads[f"beta{i}"] = affine_bn_relu_backward(dout, caches.pop())
          elif self.normalization == "layernorm":
            dout, dw, db, grads[f"gamma{i}"], grads[f"beta{i}"] = affine_ln_relu_backward(dout, caches.pop())
          else:
