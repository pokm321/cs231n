...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    x_stretch = x.reshape(N,-1) # N*D
    out = x_stretch @ w + b # N*M
...
def relu_backward(dout, cache):
    ...
    # out = x * (x > 0)
    dx = dout * (x > 0) # x > 0 일때는 upstream 그대로 전달, x <= 0 일땐 0
...
def softmax_loss(x, y):
    ...
    probability = x_exp[range(N),y]/np.sum(x_exp, axis=1)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = [eps, mean, var, x, xhat, std, gamma, beta]
...
def batchnorm_backward(dout, cache):
    ...
    eps, mean, var, x, xhat, std, gamma, beta = cache
    N, D = x.shape

    # dout/dbeta, dout/dgamma, dout/dxhat 계산
    dbeta = np.sum(dout * 1, axis=0)
    dgamma = np.sum(dout * 1 * xhat, axis=0)
    dxhat = dout * 1 * gamma # (N, D)

    # dout/d(x-mean) 계산
    dxmu1 = dxhat / std # (N, D)

    distd =  np.sum(dxhat * (x - mean), axis=0)  # (D,)  istd = 1/std
    dvar = distd * (-0.5 / std**3)               # (D,)
    dxmu_sq = dvar * np.ones((N,D)) * (1/N)      # (N, D)
    dxmu2 = dxmu_sq * 2 * (x - mean)             # (N, D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (dxhat / std) - (np.sum(dxhat, axis=0) / (N * std)) \
          - (np.sum(dxhat * xhat, axis=0) * xhat / (N * std))
...
def layernorm_backward(dout, cache):
    ...
    dx = (dxhat / std) - (np.sum(dxhat, axis=1).reshape((-1, 1)) / (D * std)) \
          - (np.sum(dxhat * xhat, axis=1).reshape((-1, 1)) * xhat / (D * std))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_mod = x.transpose((0,2,3,1)).reshape((-1,C))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    dout_mod = dout.transpose((0,2,3,1)).reshape((-1,C))
