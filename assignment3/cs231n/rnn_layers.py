...
def rnn_step_backward(dnext_h, cache):
    ...
    # next_h = np.tanh(x @ Wx + prev_h @ Wh + b)  -> forward 코드
    x, prev_h, Wx, Wh, b, next_h = cache

    dlinear = dnext_h * (1 - next_h ** 2)  # tanh'(x) = 1 - tanh(x)^2
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      ...
      h[:, t, :], c = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
      cache[t] = c   # cache = {..., t : (x, prev_h, Wx, Wh, b, next_h), ...}
...
def rnn_backward(dh, cache):
    ...
    dx, dh_sum, dWx, dWh, db = \
      np.zeros((N, T, D)), np.zeros((N, T, H)), np.zeros((D, T, H)), np.zeros((H, T, H)), np.zeros((H, T))
    
    dh_horizontal = 0 # gradient passed between timestamp
    for t in reversed(range(T)):
      dh_sum[:,t,:] = dh_horizontal + dh[:,t,:] # t+1 에서 가로로 들어오는 dh_horizontal + 세로로 전달받는 dh
      dx[:,t,:], dh_horizontal, dWx[:,t,:], dWh[:,t,:], db[:,t] = rnn_step_backward(dh_sum[:,t,:], cache[t])
